...
// Building a leaf with saturation 0.1, generating the bugs and placing them.
// Every bug you click away will increase the saturation until it's fully vibrant.
...
export default function BugCanvas({ canvasId, leafImageSrc, bugPositions }: Props) {
  ...
  const drawAll = (
    ...
    // Draw the leaf with saturation (0.1 = dull, 1 = vibrant)
    ctx.save()
    const saturation = 0.1 + 0.9 * saturationLevel // avoid going fully gray
    ctx.filter = `saturate(${saturation})`
    ctx.drawImage(leaf, 0, 0, ctx.canvas.width, ctx.canvas.height)
    ctx.restore()

    // Draw bugs with no filter
  ...
  const handleCanvasClick = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    const newSaturation = 1 - bugsRemaining / bugs.length // 0 → 1
  ...
  return (
    <canvas
      ...
      style={{ touchAction: 'manipulation' }} // helps mobile responsiveness
    />
  )
}
